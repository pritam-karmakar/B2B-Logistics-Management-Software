<?php
    ...
    if(isset($_POST['add_money_manual']))
    {
        $date = date("Y-m-d");
        extract($_POST);
        $conditionArr = array("type"=>"branches","type_id"=>$branch_id,"amount"=>$amount,"date"=>$date,"txn_id"=>$txn_id);
        if($query->insertData('add_money_requests',$conditionArr))
        {
            echo '<script type="text/javascript" language="javascript">
                    alert("Add Money Requested Successfully !!");
                    window.location = "add_money.php";
                  </script>';
        }else{
            echo '<script type="text/javascript" language="javascript">
    ...
    // create cashbook request start
    if(isset($_POST['createCashbookRequest']))
    {
        $created_at = date("Y-m-d H:i:s");
        $req_no = "CSHBKRQ".time();
        extract($_POST);
        $dataAry = array("req_no"=>$req_no,"branch_id"=>$branch_id,"lr_no"=>$lr_no,"amount"=>$amount,"ref_no"=>$ref_no,"created_at"=>$created_at);
        if($query->insertData('cashbook',$dataAry))
        {
            echo '<script type="text/javascript" language="javascript">
                    alert("Cashbook Request Created Successfully !!");
                    window.location = "cashbook_request.php";
                  </script>';
        }else{
            echo '<script type="text/javascript" language="javascript">
                    alert("Something went wrong! Please contact with administrator");
    ...
    if(isset($_POST['data']) && isset($_POST['type']) && isset($_POST['type_id']))
    {
        ...
        $get_ticket_detals = $query->getData("*","tickets",$join,$cond,"tickets`.`id","DESC","");
    ...
    if(isset($_POST['change_password']))
    {
        ...
        if($updPassword = $query->updateData('branches',$cond,'id',$us_id))
        {
            echo '<script type="text/javascript" language="javascript">
    ...
    if(isset($_POST['updateProfile'])){
        ...
        if($query->updateData('branches',$dataArr,'id',$id)){
            echo '<script type="text/javascript" language="javascript">
    ...
    if(isset($_POST['submitAppointment'])){
        extract($_POST);
        $appointArray["user_type"] = "branches";
        $appointArray["type_id"] = $branch_id;
        foreach($_POST as $varkey => $varvalue){
            // $appointArray[$varkey] = $newfunc->real_string(trim($varvalue, " "));
    ...
    if(isset($_POST['add_Order']))
    {
        ...
        $user_cond=array("id"=>$branch_id);
        $check_user_details = $query->getData("*","branches","",$user_cond,"","","");
        $credit_type = $check_user_details[0]['credit_type'];
        $credit_limt= $check_user_details[0]['credit_limit'];
        $wallet_balance = $check_user_details[0]['wallet_balance'];
        if($getBranchDetails['branch_charge'] == "no"):
            $applied_weight_charge = $weight_rate;
        elseif($getBranchDetails['branch_charge'] == "yes"):
            $getstate1 = $query->getData("`pincodes`.*,`states`.`zone_id`,`zones`.`zone`","pincodes",array("0"=>array("LEFT","states","states","id","pincodes","state_id"),array("LEFT","zones","zones","id","states","zone_id")),array("pincode"=>$pin),"","","1")[0];
            $getstate2 = $query->getData("`pincodes`.*,`states`.`zone_id`,`zones`.`zone`","pincodes",array("0"=>array("LEFT","states","states","id","pincodes","state_id"),array("LEFT","zones","zones","id","states","zone_id")),array("pincode"=>$del_pin),"","","1")[0];
            $frightZone = $getstate1['zone']."_to_".$getstate2['zone'];
            $applied_weight_charge = $query->getData('*','branches_fright_master','',array('branch_id'=>$getBranchDetails['id']),'id','DESC','1')[0][$frightZone];
        endif;
        ...
        if ($cft_type == "6CFT") {
            ...
            $cartage_charge = $_POST['cartage_charge_6cft'];
            ...
            $cartage_charge = $_POST['cartage_charge_8cft'];
            $vol_weight = $_POST['eightcft_volweight'];
            $igst_amount = $_POST['igst_8cft'];
            $cgst_amount = $_POST['cgst_8cft'];
            $sgst_amount = $_POST['sgst_8cft'];
            ...
            $cartage_charge = $_POST['cartage_charge_10cft'];
            $vol_weight = $_POST['tencft_volweight'];
            $igst_amount = $_POST['igst_10cft'];
            $cgst_amount = $_POST['cgst_10cft'];
            $sgst_amount = $_POST['sgst_10cft'];
        }
        ...
        if($payment_mode=='Prepaid' || $payment_mode=="CoD")
        {
            ...
            {
                if($credit_type == "TBB" && $credit_limt >= round(($total_charge - $wallet_balance), 2)){
                    $order_done = 10;
        ...
        if($order_done == 0)
        {
            echo'<script>alert("Sorry!!Order not created due to unsufficient balance.");window.location="add_money";</script>';
        }
        else
        {
            ...
            for($i=0;$i<count($qty);$i++)
            {
                $dimensions[] = array(
                    ...
                    "count" => floatval($qty[$i])
                );
                $count_suborders = $count_suborders+floatval($qty[$i]);
            }
            ...
            if($ref_no =='')
            {
                $subident = "KINGBR".rand(100000,999999);
            }
            else
            {
                $subident = $ref_no;
                ...
                if(!empty($jwt)){
                    ...
                    if($error_j){
                        ...
                        if(isset($response_arr->job_id) && !empty($response_arr->job_id))
                        {
                            ...
                            $congsigner_arr = array("order_id"=>$order_id,"company"=>$consigner_company,"name"=>$consigner_name,"phone"=>$consigner_phone,"email"=>$consigner_email,"address"=>$consigner_address,"city"=>$consigner_city,"state"=>$consigner_state);
                            $insert_consigner_details =  $query->insertData('consigner_details',$congsigner_arr);
                            for($i=0;$i<count($qty);$i++)
                            {
                                $cong_box = array("order_id"=>$order_id,"qty"=>floatval($qty[$i]),"length"=>floatval($length[$i]),"width"=>floatval($width[$i]),"height"=>floatval($height[$i]));
                                ...
                                if($insert_transaction)
                                {
                                    ...
                                    if($up_wall){
                                        if($order_done == 10){
                                            ...
                                            $transscrArr = array("date_time"=>$tdate,"user_type"=>"branches","type_id"=>$branch_id,"amount"=>$creditUse,"balance"=>$new_bal,"details"=>$crdetails,"txn_id"=>$CRmerchantTransactionId);
                            ...
                            if($create_order && $insert_consignee_details && $insert_consigner_details && $insert_box_details && $insert_invoice_details)
                            {
                                sleep(30);
    ...
    // users order
    if(isset($_POST['add_Users_Order'])):
        $newPost = array();
        foreach($_POST as $postkey => $postval):
            $newPost[$postkey] = str_replace("'","\'",$postval);
        endforeach;
        ...
        $user_cond = array("username"=>$orderForUser);
        ...
        if($check_user_details[0]['fright_charge'] == "no"):
            $applied_weight_charge = $query->getData('*','default_fright_master','',array('id'=>'1'),'id','DESC','1')[0][$frightZone];
        elseif($check_user_details[0]['fright_charge'] == "yes"):
            $applied_weight_charge = $query->getData('*','users_fright_master','',array('user_id'=>$check_user_details[0]['id']),'id','DESC','1')[0][$frightZone];
        endif;
                    ...
                    if($error_j){
                      echo $error_j;
                        ...
                        if(isset($response_arr->job_id) && !empty($response_arr->job_id))
                        {
                            ...
                            $orderArr = array("user_type"=>"users","type_id"=>$check_user_details[0]['id'],"order_id"=>$order_id,"order_date"=>$order_date,"order_time"=>$order_time,"warehouse_id"=>$warehouse_id,"pick_pin"=>$pin,"del_pin"=>$del_pin,"weight"=>$weight,"vol_weight"=>$vol_weight,"payment_mode"=>$payment_mode,"cod_amount"=>$cod_amnt,"profit_amount"=>$profit_amount,"invoice_amount"=>$invoice_amount,"insurance"=>$insurance,"ltype"=>$ltype,"lr"=>$lr,"description"=>$description,"job_id"=>$job_id,"hsn"=>$hsn,"oda"=>$oda,"cod_charge"=>$cod_charge,"fuel_surcharge"=>$fuel_surcharge,"awb_charge"=>$awb_charge,"fob_surcharge"=>$fob_surcharge,"handeling_charge"=>$handeling_charge,"pickup_charge"=>$pickup_charge,"damage_surcharge"=>$damage_surcharge,"oda_surcharge"=>$oda_surcharge,"packaging_surcharge"=>$packaging_surcharge,"special_delivery_charge"=>$special_delivery_charge,"gst_charge"=>$gst_charge,"weight_charge"=>$weight_charge,"total_charge"=>$total_charge,"channel"=>"Delhivery","item_type"=>$item,"seller_gst_tin"=>$seller_gst_tin,"consignee_gst_tin"=>$consignee_gst_tin,"status"=>"Created","cft_type"=>$cft_type,"subident"=>$subident,"origin_center"=>$origins[0],"destination_center"=>$destinations[0],"origin_city"=>$origins[1],"destination_city"=>$destinations[1],"applied_weight_charge"=>$applied_weight_charge,"applied_fuel_surcharge"=>$applied_fuel_surcharge,"applied_fuel_surcharge_type"=>$applied_fuel_surcharge_type,"applied_cod_charge_min"=>$applied_cod_charge_min,"applied_cod_charge"=>$applied_cod_charge,"applied_cod_charge_type"=>$applied_cod_charge_type,"applied_awb_charge"=>$applied_awb_charge,"applied_fob_surcharge_minimum"=>$applied_fob_surcharge_minimum,"applied_fob_surcharge_percentage"=>$applied_fob_surcharge_percentage,"applied_handeling_charge"=>$applied_handeling_charge,"applied_handeling_charge_type"=>$applied_handeling_charge_type,"applied_damage_surcharge_min"=>$applied_damage_surcharge_min,"applied_damage_surcharge"=>$applied_damage_surcharge,"applied_damage_surcharge_type"=>$applied_damage_surcharge_type,"applied_oda_surcharge_min"=>$applied_oda_surcharge_min,"applied_oda_surcharge"=>$applied_oda_surcharge,"applied_oda_surcharge_type"=>$applied_oda_surcharge_type,"applied_packaging_surcharge"=>$applied_packaging_surcharge,"applied_packaging_surcharge_type"=>$applied_packaging_surcharge_type,"applied_special_delivery_or_appointment_charge_min"=>$applied_special_delivery_or_appointment_charge_min,"applied_special_delivery_or_appointment_charge"=>$applied_special_delivery_or_appointment_charge,"applied_special_delivery_or_appointment_charge_type"=>$applied_special_delivery_or_appointment_charge_type,"applied_pickup_charge"=>$applied_pickup_charge,"applied_pickup_charge_type"=>$applied_pickup_charge_type,"order_created_by"=>$getBranchDetails['type'],"order_created_by_id"=>$getBranchDetails['id']);
                            $create_order = $query->insertData('orders',$orderArr);
                            ...
                            if($payment_mode=='Prepaid' || $payment_mode=="CoD"){
                                ...
                                $neworderdetails = str_replace("'", "\'", "Order id.:".$order_id."'s new order placed");
              		            $transsArr = array("date_time"=>$tdate,"user_type"=>"users","user_id"=>$check_user_details[0]['id'],"amount"=>$total_charge,"balance"=>$new_bal,"type"=>"Online","details"=>$neworderdetails,"txn_id"=>$merchantTransactionId);
                  	            $insert_transaction = $query->insertData('transactions',$transsArr);
                                if($insert_transaction)
                                {
                                    $cond_uw= array("wallet_balance"=>$new_bal);
                                    $up_wall = $query->updateData('users',$cond_uw,'id',$check_user_details[0]['id']);
                                    if($up_wall){
                                        if($order_done == 10){
                                            $getprevCRTrans = $query->getData('*','credit_transactions','','','id','DESC','1');
                                          	if($getprevCRTrans != 0){
                                            ...
                                            $crdetails = str_replace("'", "\'", "Order id.:".$order_id."'s order placed");
                                            $transscrArr = array("date_time"=>$tdate,"user_type"=>"users","type_id"=>$check_user_details[0]['id'],"amount"=>$creditUse,"balance"=>$new_bal,"details"=>$crdetails,"txn_id"=>$CRmerchantTransactionId);
                              	            $insertcr_transaction = $query->insertData('credit_transactions',$transscrArr);
                              	        }
                                    }else{
                                        echo '<script type="text/javascript" language="javascript">
                                ...
                                $transsArrTP = array("date_time"=>$tdate,"user_type"=>'users',"user_id"=>$check_user_details[0]['id'],"amount"=>'0',"balance"=>$wallet_balance,"type"=>"Manual","details"=>$descTP,"txn_id"=>$merchantTransactionIdTP);
                  	            $insert_transactionTP = $query->insertData('transactions',$transsArrTP);
                            }elseif($payment_mode=='Franchise-ToPay'){
                                $getprevTransFTP = $query->getData('*','transactions','',array('txn_id_type'=>'Serial'),'id','DESC','1');
                              	if($getprevTransFTP != 0){
                                ...
                                $transsArrFTP = array("date_time"=>$tdate,"user_type"=>'users',"user_id"=>$check_user_details[0]['id'],"amount"=>'0',"balance"=>$wallet_balance,"type"=>"Manual","details"=>$descFTP,"txn_id"=>$merchantTransactionIdFTP);
                  	            $insert_transactionFTP = $query->insertData('transactions',$transsArrFTP);
                            }
