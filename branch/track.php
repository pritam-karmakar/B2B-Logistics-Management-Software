...
if(isset($_GET['lrn'])){
    ...
    $lrn = $_GET['lrn'];
    $order_details = $query->getData("*, (SELECT SUM(`qty`) FROM `box_details` WHERE `order_id` = `orders`.`order_id`) as 'total_boxes'","orders",[array("LEFT","consignee_details","consignee_details","order_id","orders","order_id")],array("user_type"=>"branches","type_id"=>$user_id,"lr"=>$lrn),"","","");
    if($order_details != 0){
