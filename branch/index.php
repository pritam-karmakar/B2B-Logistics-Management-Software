<?php
    ...
    {
        $username = $newfunc->RemoveSpecialChar($newfunc->real_string($_POST['username']));
        $password = md5($newfunc->RemoveSpecialChar($newfunc->real_string($_POST['password'])));
        $cond = array("branch_user_name"=>$username);
        $getlogin = $newquery->getData("*","branches","",$cond,"","","");
        if($getlogin != 0){
            if ($password === $getlogin[0]['password']) 
            {
                $_SESSION['branchusername'] = $getlogin[0]['branch_user_name'];
                $_SESSION['branchuser_id'] = $getlogin[0]['id'];
...
<html lang="en" class="light-style layout-wide  customizer-hide" dir="ltr" data-theme="theme-default" data-assets-path="../assets/" data-template="vertical-menu-template">
    ...
    <title>B2B Branch Login || Kingfish Logistics</title>
  ...
  <div class="authentication-inner row m-0">
    ...
    <div class="d-flex col-12 col-lg-5 col-xl-4 align-items-center authentication-bg p-sm-5 p-4">
      <div class="w-px-400 mx-auto">
        ...
        <!-- /Logo -->
        <h4 class="mb-2">Welcome to Kinshfish Logistics! 👋</h4>
        <p class="mb-4">Please sign-in to your account and start the adventure</p>

        <form id="formAuthentication" class="mb-3" action="index" method="POST" >
          <div class="mb-3">
            <label for="username" class="form-label">BranchUsername</label>
