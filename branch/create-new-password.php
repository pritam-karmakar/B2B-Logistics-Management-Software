...
if(empty($_GET['success']) && $_GET['success'] != "1" && $_SESSION['sent_otp_email_type'] != "branches" && empty($_SESSION['sent_otp_email_id'])):
...
if(isset($_POST['saveChanges']) && !empty($_POST['password']) && !empty($_POST['confirm_password'])){
    ...
    if(strlen($password) >= 6 && strlen($confirm_password) >= 6):
        if($password === $confirm_password):
            ...
            $updOTP = $newquery->updateData("branches",array("password"=>$newPassword),"id",$_SESSION['sent_otp_email_id']);
            if($updOTP):
                unset($_SESSION['sent_otp_email_type']);
                unset($_SESSION['sent_otp_email_id']);
                $newfunc->alertRedirect("Password successfully changed","https://b2b.kingfishlogistics.in/branch/");
            else:
