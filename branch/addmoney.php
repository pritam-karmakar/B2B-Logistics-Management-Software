...
if(isset($_POST['add_money']))
{
    ...
    $redirectUrl = 'https://kingfishlogistics.in/b2b/branch/addmoney?jwtForPayment='.$jwt;
...
elseif(isset($_GET['jwtForPayment'])){
    ...
    if(base64_encode(hash_hmac('SHA256', $jwt[0].$jwt[1], '89ba7c48-8e3a-475c-b420-2aebda80d6c8')) == $jwt[2]){
        $get_user_details = $query->getData("*","branches","",array("id"=>$id),"","","");
        $_SESSION['branchusername'] = $get_user_details[0]['branch_user_name'];
        $_SESSION['branchuser_id'] = $get_user_details[0]['id'];
        ...
        curl_close($curl);
        if($err){
            $query->insertData('gateways_error_transactions',array('user_type'=>'branches','type_id'=>$id,'amount'=>$amnt,'transaction_id'=>$txn_id,'date_time'=>date('Y-m-d H:i:s'),'gateway'=>'Phone Pe'));
...
elseif(isset($_POST['add_money_gateway'])){
    ...
    header("location:https://b2b.kingfishlogistics.in/branch/");
    exit();
}
...
<html lang="en" class="light-style layout-menu-fixed" dir="ltr" data-theme="theme-default" data-assets-path="../assets/" data-template="vertical-menu-template-free">
  <head>
    ...
    <title>Add Money Redirecting - B2B Branch || Kingfish Logistics</title>
