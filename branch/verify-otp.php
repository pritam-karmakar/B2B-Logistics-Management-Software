...
if(empty($_GET['done']) && $_GET['done'] != "1" && $_SESSION['sent_otp_email_type'] != "branches" && empty($_SESSION['sent_otp_email_id'])):
...
if(isset($_POST['verify_otp']) && !empty($_POST['entered_otp'])){
    ...
    $validation = $newquery->getData("*","branches","",array("id"=>$_SESSION['sent_otp_email_id']),"id","DESC","1");
    if($validation != 0):
        if($validation[0]['verification_otp'] == $entered_otp):
            $updOTP = $newquery->updateData("branches",array("verification_otp"=>0),"id",$_SESSION['sent_otp_email_id']);
            if($updOTP):
