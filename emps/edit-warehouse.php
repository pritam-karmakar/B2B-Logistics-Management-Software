...
if(!empty($_GET['warehouse_id'])){
    ...
    $get_warehouse = $query->getData("*","warehouses","",array("id"=>$warehouse_id),"id","DESC","1")[0];
    $user_id = $get_warehouse['type_id'];
    $getuser = $query->getData("*",$get_warehouse['type'],"",array("id"=>$user_id),"id","DESC","1")[0];
    if(isset($_POST['edit_warehouse'])){
        extract($_POST);
        $name = $get_warehouse['warehouse_name'];
        $table='warehouses';
        ...
        $condition = array("type"=>$get_warehouse['type'],"type_id"=>$user_id,"pincode"=>$pin,"city"=>$city,"state"=>$state,"country"=>$country,"address"=>$address,"contact_person"=>$contact_person,"email"=>$email,"phone_number"=>$phone_number,"same_return_address"=>$return_address_same,"r_address"=>$r_address,"r_pin"=>$r_pin,"r_city"=>$r_city,"r_state"=>$r_state,"r_country"=>$r_country,"pickup_slots"=>$pickup_slots);
        
        if($cft == 'all'){
            $get_3pl = $query->getData("*","3pls","",'',"","","");
            $successCount = 0;
            foreach($get_3pl as $threepl){
                ...
                if($error){
                  echo $error;
                    ...
                    $cf = curl_init();
                    curl_setopt($cf, CURLOPT_URL, "https://track.delhivery.com/api/backend/clientwarehouse/edit/");
                    curl_setopt($cf, CURLOPT_POSTFIELDS, json_encode($fields));
                    ...
                    if($error){
                      echo $error;
                        ...
                        if($success == 1){
                            ...
                            $newfunc->alertRedirect($error_message,$_SERVER['HTTP_REFERER']);
                        }
                    }
                }
            }
            if($successCount == count($get_3pl)){
                ...
                if($update){
                    $newfunc->alertRedirect('You have successfully updated the ware house','ware-house-master');
                }else{
                    $newfunc->alertRedirect('Warehouse updation failed! Contact with administrator now!','ware-house-master');
                }
            }
        }else{
            $cond =array("id"=>$cft);
            ...
            if($error){
              echo $error;
                ...
                $ct = curl_init();
                curl_setopt($ct, CURLOPT_URL, "https://track.delhivery.com/api/backend/clientwarehouse/edit/");
                curl_setopt($ct, CURLOPT_POSTFIELDS, json_encode($fields));
                ...
                if($error){
                  echo $error;
                    ...
                    if($success == 1){
                        ...
                        $newfunc->alertRedirect(ucwords($error_message),'ware-house-master');
                    }
                }
            }
        }
...
?>
    <div class="content-body">
        ...
        <div class="container-fluid">
            <div class="card">
                <form action"edit_warehouse?warehouse_id=<?= $get_warehouse['id']; ?>" method="POST" onsubmit="return checkPincode()">
    			    <div class="card-header">
    			        <h3 class="card-title">Edit <?php if($get_warehouse['type'] == "users"){ echo $getuser['party_name']." (".$getuser['username'].")"; }elseif($get_warehouse['type'] == "branches"){ echo $getuser['branch_name']." (".$getuser['branch_user_name'].")"; } echo "<span style='text-transform: lowercase;'>'s</span>"; ?> Ware house named: <?= $get_warehouse['warehouse_name']; ?></h3>
    			    </div>
                    <div class="card-body">
                        <div class="row mb-2">
                            <div class="col-md-8 col-12">
                                ...
                                <input type="text" id="name" class="form-control" placeholder="Enter pickup point name" value="<?= $get_warehouse['warehouse_name']; ?>" readonly>
                            </div>
                        ...
                        <div class="row mb-2">
                            <div class="col-12">
                                ...
                                if($get_warehouse['same_return_address']=='Y')
                                {
                                    ?>
                                        <div class="form-group" style="display: flex;">
                                            <input type="checkbox" id="return_address_same" id="return_address_same" class="branchwise" checked>&nbsp;&nbsp;&nbsp;
                                            <input  type="hidden" id="return_address_same_hidden" name="return_address_same" value="Y">
                                            <label for="return_address_same" style="cursor: pointer;">Return address same as pickup address</label>
                                        </div>
                                    <?php
                                }
                                else
                                {
                                    ?>
                                        <div class="form-group" style="display: flex;">
                                            <input type="checkbox" id="return_address_same" id="return_address_same" class="branchwise">&nbsp;&nbsp;&nbsp;
                        ...
                        <div class="card-footer d-flex justify-content-end p-0 pt-3">
                          <button type="submit" class="btn btn-warning btn-sm shadow me-1" name="edit_warehouse">Save Changes <i class="bi bi-pen-fill"></i></button>
                        </div>
                    </div>
                </form>
            </div>
...
<?php
    ...
    header("location: ware-house-master");
}
?>
