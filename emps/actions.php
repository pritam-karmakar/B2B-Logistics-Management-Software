...
if(isset($_POST['add_user_submit'])){
    ...
    $forPartyUsernameAfter = strtoupper(str_replace(" ", "", explode(" ", $party_name, 1)[1]));
    for($i = 0; $i < $nameCount; $i++){
        $username = $partyFirstName.substr($forPartyUsername, 0, $i).$threePLType."B2BC";
        if($newquery->getData('*','users','',array('username'=>$user_name),'id','DESC','1') == 0){
            $username = $username;
            break;
        }
    }
...
if(isset($_POST['updateProfile'])){
    extract($_POST);
    $dataArr = array("employee_name"=>$name,"mobile_no"=>$mobile,"email"=>$email,"address"=>$address);
...
if(isset($_POST['changePassword']))
{
    ...
    if($updPassword = $newquery->updateData('employees',$cond,'id',$id))
    {
        echo '<script type="text/javascript" language="javascript">
...
if(!empty($_GET['mailPdf']) && $_GET['mailPdf'] == "done" && !empty($_GET['redirect'])){
    ...
    if($allotedInvoiceN != 0):
        ...
        $non_serial_invoice = $newquery->getData("*","stationary_invoices","",array("type"=>"Non Serial"),"id","DESC","1");
        $nonSerialInv = $newquery->getData("*","non_serial_invoice_prefix","","","id","DESC","1")[0];
        if(preg_replace('/[0-9]+/', '', $non_serial_invoice[0]['invoice_no']) == $nonSerialInv['invoice_prefix']):
            if($non_serial_invoice != 0):
                $invoice = $nonSerialInv['invoice_prefix'].str_replace($nonSerialInv['invoice_prefix'], "", $non_serial_invoice[0]['invoice_no'])+1;
    ...
    $insertInvArr = array('user_type'=>$entityType,'type_id'=>$id,'invoice_date'=>$inv_date,'stationary_prefix_id'=>$allotedInvoiceN['stationary_id'],'invoice_no'=>$invoice,'lrs'=>$insertLRs,'name'=>$insertName,'address'=>$insertAddress,'gst_before_amount'=>$subTotalCost,'igst'=>$igst,'sgst'=>$sgst,'cgst'=>$cgst,'grand_total'=>$grandTotal);
