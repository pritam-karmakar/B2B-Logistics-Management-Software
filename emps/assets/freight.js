    ...
    let dimension_row = `<div class="row pt-2" id="row`+diemension_no+`">
            ...
            <div class="col-lg-1 d-flex justify-content-end align-items-center" style="padding-top: 1.5rem;">  
                <div class="text-center">
                    <button id="remove_weight`+diemension_no+`" type="button" class="btn btn-danger btn-sm"><i style="font-size: 17px;" class="bi bi-dash-circle-fill"></i></button>
                </div> 
            </div>
        </div>`;
...
$('button[name=CalculateFreight]').click(function(){
    ...
    const userType = $('select[name=user-type]').val();
    if(userType == '' || userType == 0){
        $('.user-type-error').html('Invalid user type!');
        isValid = false;
    }else{
        $('.user-type-error').html('');
    }
    const UsersorBranches = $('select[name=UsersorBranches]').val();
    if(UsersorBranches == '' || UsersorBranches == 0){
        $('.UsersorBranches-error').html('Invalid User/Branch!');
        isValid = false;
    }else{
        $('.UsersorBranches-error').html('');
    }
    ...
    if(isValid != false){
        ...
        $('.freight-calculation-card').parent('.card').prepend(loader());
        setTimeout(function(){
            $.ajax({
                type: 'POST',
                url: 'return-actions',
                data: {
                    'userType': userType,
                    'UsersorBranches': UsersorBranches,
...
$('select[name=user-type]').change(function(){
    ...
    $.ajax({
        type: 'post',
        url: 'act',
        data: 'freightUserType='+UserType,
        success: function(result){
            backNormalBody();
            $('select[name=UsersorBranches]').html(result);
        }
    });
})
