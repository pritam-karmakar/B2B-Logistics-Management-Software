<?php
    ...
    if(isset($_POST['pick_pin']) && isset($_POST['del_pin'])  && isset($_POST['type']) && isset($_POST['type_id']) && isset($_POST['volweight']))
    {
        ...
        if(!empty($pick_zone) && !empty($del_zone)){
            ...
            $get_type_details = $query->getData('fright_charge',"users","",array("id"=>$type_id),"","","")[0];
            
            if($get_type_details == 'yes')
            {
                $get_freight_zone = $query->getData($freight_zone,"users_fright_master","",array("user_id"=>$type_id),"","","");
            }
            else
            {
    ...
    && isset($_POST['tencft_volweight']))
    {
        ...
        if(!empty($pick_zone) && !empty($del_zone)){
            ...
            $f_cost_array = array($freight_cost_sixcft_volweight,$freight_cost_eightcft_volweight,$freight_cost_tencft_volweight);
