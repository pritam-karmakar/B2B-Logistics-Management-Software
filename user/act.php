        ...
        if(!empty($jwt)){
            ...
            if($error){
                ...
                if(!empty($result->pickup_id) && !empty($result->incoming_center_name)){
                    $requestArr['type'] = "users";
                    $requestArr['type_id'] = $user_id;
...
if(isset($_POST['uploadBulkSubmit']) && !empty($_POST['warehouse']) && !empty($_FILES['upload_order_file']['name'])){
    ...
    if($ext == 'xlsx' || $ext == 'csv'){
        ...
        foreach($obj as $sheet){
            if($sl != 1){
                $weight = $sheet[14];
                $check_user_details = $query->getData("*","users","",array("id"=>$user_id),"","","");
                $party_type = $check_user_details[0]['party_type'];
                $credit_limit= $check_user_details[0]['credit_limit'];
                ...
                $get_user_details = $check_user_details[0];
                
                if($get_user_details['fright_charge'] == "yes"){
                    $fright = $query->getData('*','users_fright_master','',array('user_id'=>$user_id),'id','DESC','1')[0];
                }else{
                ...
                $awb_charge = $get_user_details['awb_charge'];
                if($sheet[22] == "No"){
                    $fob_surcharge = $get_user_details['fob_surcharge_minimum'];
                }elseif($sheet[22] == "Yes"){
                    $fob_surcharge = floatval($sheet[18]*($get_user_details['fob_surcharge_percentage']/100),2);
                }
                if($get_user_details['fuel_surcharge_type'] == "Fixed"){
                    $fuel_surcharge = floatval($get_user_details['fuel_surcharge']);
                }elseif($get_user_details['fuel_surcharge_type'] == "Percentage"){
                    $fuel_surcharge = floatval($frightCharge*($get_user_details['fuel_surcharge']/100));
                }
                if($get_user_details['handeling_charge_type'] == "Quantity"){
                    $handeling_charge = floatval($handeling_charge*$sheet[10]);
                }elseif($get_user_details['handeling_charge_type'] == "Kg"){
                    $handeling_charge = floatval($handeling_charge*$weight);
                }
                if($get_user_details['damage_surcharge_type'] == "Quantity"){
                    $new_damage_surcharge = floatval($get_user_details['damage_surcharge']*$sheet[10]);
                    if($get_user_details['damage_surcharge_min'] < $new_damage_surcharge){
                        $damage_surcharge = $new_damage_surcharge;
                    }else{
                        $damage_surcharge = $get_user_details['damage_surcharge_min'];
                    }
                }elseif($get_user_details['damage_surcharge_type'] == "Kg"){
                    $new_damage_surcharge = floatval($get_user_details['damage_surcharge']*$weight);
                    if($get_user_details['damage_surcharge_min'] < $new_damage_surcharge){
                        $damage_surcharge = $new_damage_surcharge;
                    }else{
                        $damage_surcharge = $get_user_details['damage_surcharge_min'];
                ...
                if($get_user_details['oda_surcharge_type'] == "Quantity"){
                    $new_oda_surcharge = floatval($get_user_details['oda_surcharge']*$sheet[10]);
                    if($get_user_details['oda_surcharge_min'] < $new_oda_surcharge){
                        $oda_surcharge = $new_oda_surcharge;
                    }else{
                        $oda_surcharge = $get_user_details['oda_surcharge_min'];
                    }
                }elseif($get_user_details['oda_surcharge_type'] == "Kg"){
                    $new_oda_surcharge = floatval($get_user_details['oda_surcharge']*$weight);
                    if($get_user_details['oda_surcharge_min'] < $new_oda_surcharge){
                        $oda_surcharge = $new_oda_surcharge;
                    }else{
                        $oda_surcharge = $get_user_details['oda_surcharge_min'];
                ...
                if($get_user_details['packaging_surcharge_type'] == "Quantity"){
                    $packaging_surcharge = floatval($packaging_surcharge*$sheet[10]);
                }elseif($get_user_details['packaging_surcharge_type'] == "Kg"){
                    $packaging_surcharge = floatval($packaging_surcharge*$weight);
                }
                if($get_user_details['special_delivery_or_appointment_charge_type'] == "Fixed"){
                    $new_special_delivery_charge = floatval($get_user_details['special_delivery_or_appointment_charge']);
                    if($get_user_details['special_delivery_or_appointment_charge_min'] < $new_special_delivery_charge){
                        $special_delivery_charge = $new_special_delivery_charge;
                    }else{
                        $special_delivery_charge = $get_user_details['special_delivery_or_appointment_charge_min'];
                    }
                }elseif($get_user_details['special_delivery_or_appointment_charge_type'] == "Percentage"){
                    $new_special_delivery_charge = floatval($frightCharge*($get_user_details['special_delivery_or_appointment_charge']/100));
                    if($get_user_details['special_delivery_or_appointment_charge_min'] < $new_special_delivery_charge){
                        $special_delivery_charge = $new_special_delivery_charge;
                    }else{
                        $special_delivery_charge = $get_user_details['special_delivery_or_appointment_charge_min'];
                ...
                if($get_user_details['pickup_charge_type'] == "Quantity"){
                    $pickup_charge = floatval($pickup_charge*$sheet[10]);
                }elseif($get_user_details['pickup_charge_type'] == "Kg"){
                    $pickup_charge = floatval($pickup_charge*$weight);
                }
                $cartage_charge = 0;
                $cod_amnt = 0;
                $cod_charge = 0;
                $payment_mode = $sheet[16];
                if($payment_mode == "C" && ($check_user_details[0]['cod_charge_enable_disable'] == "enable")){
                    $payment_mode = "CoD";
                    $payment_type="CoD";
                    $cod_amnt = round(($sheet[17]),2);
                    if($get_user_details['cod_charge_type'] == "Fixed"){
                        $new_cod_charge = $get_user_details['cod_charge_type'];
                        ...
                        $new_cod_charge = $frightCharge*($get_user_details['cod_charge_type']/100);
                        if($get_user_details['cod_charge_min'] < $new_cod_charge){
                            $cod_charge = $new_cod_charge;
                        }else{
                            $cod_charge = $get_user_details['cod_charge_min'];
                ...
                if($payment_mode == "Prepaid" || $payment_mode == "CoD"){
                    if($total_charge > $wallet_balance){
                        if($party_type == "TBB" && $credit_limit >= round(str_replace("-", "", ($total_charge - $wallet_balance)), 2)){
                            $order_done = 10;
                        ...
                        if(!empty($jwt)){
                            ...
                            if($error_j){
                                ...
                                if(isset($response_arr->job_id) && !empty($response_arr->job_id))
                                {
                                    ...
                                    $cong_box = array("order_id"=>$order_id,"qty"=>intval($sheet[10]),"length"=>floatval($sheet[11]),"width"=>floatval($sheet[12]),"height"=>floatval($sheet[13]));
                                    ...
                                    if($payment_mode == "CoD" || $payment_mode == "Prepaid"){
                                        ...
                                        $newdetails = str_replace("'", "\'", "Order id.:".$order_id."'s new order placed");
                      		            $transsArr = array("date_time"=>$tdate,"user_type"=>"users","user_id"=>$user_id,"amount"=>$total_charge,"balance"=>$new_bal,"type"=>"Online","details"=>$newdetails,"txn_id"=>$merchantTransactionId);
                                        ...
                                        if($insert_transaction)
                                        {
                                            ...
                                            if($up_wall){
                                                if($order_done == 10){
                                                    ...
                                                    $crdetails = str_replace("'", "\'", "Order id.:".$order_id."'s new order placed");
                                                    $transscrArr = array("date_time"=>$tdate,"user_type"=>"users","type_id"=>$user_id,"amount"=>$creditUse,"balance"=>$new_bal,"details"=>$crdetails,"txn_id"=>$CRmerchantTransactionId);
                                      	            $insert_transaction2 = $query->insertData('credit_transactions',$transscrArr);
                                      	        }
                                            }else{
                                                echo '<script type="text/javascript" language="javascript">
                                        ...
                                        $descTP = str_replace("'", "\'", "Order id.: ".$order_id."'s new order placed with to pay");
                                        $transsArrTP = array("date_time"=>$tdate,"user_type"=>'users',"user_id"=>$user_id,"amount"=>'0',"balance"=>$wallet_balance,"type"=>"Manual","details"=>$descTP,"txn_id"=>$merchantTransactionIdTP);
                          	            ...
                          	            if($insert_transactionTP){
                                            $query->updateData('users',array('to_pay_due'=>floatval($total_charge+$get_user_details['to_pay_due'])),'id',$user_id);
                          	            }
                                        ...
                                        $descFTP = str_replace("'", "\'", "Order id.: ".$order_id."'s new order placed with franchise to pay");
                                        $transsArrFTP = array("date_time"=>$tdate,"user_type"=>'users',"user_id"=>$user_id,"amount"=>'0',"balance"=>$wallet_balance,"type"=>"Manual","details"=>$descFTP,"txn_id"=>$merchantTransactionIdFTP);
                          	            ...
                          	            if($insert_transactionFTP){
                                            $query->updateData('users',array('franchise_topay_due'=>floatval($total_charge+$get_user_details['franchise_topay_due'])),'id',$user_id);
                          	            }
                                    }
                                          ...
                                          echo $error_login;
                                            ...
                                            if($errors){
                                              echo $errors;
                                                ...
                                                $update_recent_order = $query->updateData('orders',$cond_ord,'order_id',$jobArr[1]);
                                            }
                                        }
                                    }
                                }else{
...
function updateLRstatus($cft_id,$jobArr,$query){
    $get_3pl = $query->getData("*","3pls","",array("id"=>$cft_id),"","","")[0];
    $registered_name = $get_3pl['registered_name'];
    $password = $get_3pl['password'];
    $loginF = array(
