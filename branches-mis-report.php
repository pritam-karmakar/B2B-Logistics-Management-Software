...
$logfile = $baseDir.'branchLogFile.log';
file_put_contents($logfile, "Branch's MIS Script started at " . date('Y-m-d H:i:s') . "\n", FILE_APPEND);
...
try {
    ...
    if ($getUsersBranches != 0) {
        foreach ($getUsersBranches as $usersbranches) {
            ...
            if ($order_details != 0) {
                ...
                $messagename = $usersbranches['branch_name'];
                $filename = date("d-m-Y") . '_mis_report_of_' . $usname . '.csv';
                $file = $baseDir.'assets/' . $filename;
                ...
                if (mail($mailto, $subject, $nmessage, $header)) {
                    if (unlink($file)) {
                        sleep(15);
                        unset($report);
                        file_put_contents($logfile, $usname . "'s MIS report successfully sent.\n", FILE_APPEND);
                    }
                }
            }
        }
    ...
    file_put_contents($logfile, "\nBranch's MIS Error: " . $e->getMessage() . "\n", FILE_APPEND);
}

file_put_contents($logfile, "\nBranch's MIS Script ended at " . date('Y-m-d H:i:s') . "\n", FILE_APPEND);
?>
