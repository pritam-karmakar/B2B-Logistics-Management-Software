...
if(isset($_GET['bill_report_entity_type']) && isset($_GET['bill_report_entity_id'])){
    ...
    if(count($lrs) > 1){
        $startDate = $newquery->getData('*','orders','',array('lr'=>end($lrs)),'id','DESC','1')[0]['order_date'];
        $endDate = $newquery->getData('*','orders','',array('lr'=>reset($lrs)),'id','DESC','1')[0]['order_date'];
    }
    $entity = $newquery->getData("*",$entityType,"",$condEntity,"","","")[0];
    if($entityType == 'users'){
    ...
    if($allotedInvoiceN != 0):
        ...
        elseif($allotedInvoiceN['number_status'] == "Running"):
            $validity = 1;
            $getinvoiceD = $newquery->getData('*','stationary_invoices','',array('invoice_allotment_id'=>$allotedInvoiceN['id']),'id','DESC','1');
            ...
            if($validity == 0):
                $updinvN = $newquery->updateData("stationary_invoice_allotments",array("number_status"=>"End"),"id",$allotedInvoiceN['id']);
                if($updinvN):
    ...
    // $getinvoiceD = $newquery->getData('*','stationary_invoices','',array('stationary_prefix_id'=>$entity['stationary_id']),'id','DESC','1');
    // $newquery->getData('*','stationary_invoices','',array('stationary_prefix_id'=>$entity['stationary_id']),'id','DESC','1');
    // if($getinvoiceD != 0){
    //     $invoice = '#'.$entity['stationary_prefix'].(str_replace("#".$entity['stationary_prefix'],"",$getinvoiceD[0]['invoice_no'])+1);
    // }else{
    //     $invoice = '#'.$entity['stationary_prefix'].'10000';
    // }
    ...
    foreach($getreport as $report){
        $tqtyBox = $newquery->getData("SUM(`qty`) AS 'tqty'","box_details","",array("order_id"=>$report['order_id']),"","","");
        foreach($tqtyBox as $tqtyBx){
            $totalQty = $totalQty + $tqtyBx['tqty'];
        }
    ...
    $unitCost = round(($subTotalCost/$actualTotalQty), 2);
    if($entityType == "branches"){
        if($getreport[0]['payment_mode'] == "Prepaid" || $getreport[0]['payment_mode'] == "CoD"){
            $branchOrdersFor = $newquery->getData("*","consigner_details","",array('order_id'=>$getreport[0]['order_id']),"id","DESC","1")[0];
        }else{
            $branchOrdersFor = $newquery->getData("*","consignee_details","",array('order_id'=>$getreport[0]['order_id']),"id","DESC","1")[0];
        }
    }
    $qr_code = $newquery->getData("*","bank_details","","","id","DESC","1")[0]['qr_code'];
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <!-- Site Title -->
  <title>Kingfish Logistics Bill Report</title>
  <!--<link rel="stylesheet" href="https://b2b.kingfishlogistics.in/invoice/assets/css/style.css">-->
</head>
...
<body>
        ...
        <div class="tm_invoice_in">
          <div class="tm_invoice_head tm_mb20">
            <div class="tm_invoice_left col-3">
              <div class="tm_logo">
                <img src="https://b2b.kingfishlogistics.in/invoice/assets/logo/logo.png" alt="Logo">
              </div>
            </div>
          ...
          <div class="tm_invoice_info tm_mb10">
            <div class="tm_invoice_info_left">
              <?php if($entityType == "users"){ ?>
                  <p class="tm_mb2"><b>Bill To:</b></p>
                    <b class="tm_primary_color"><?= $uname; ?></b>
                  <p>
              ...
              <?php }elseif($entityType == "branches"){ ?>
                  <p class="tm_mb2"><b>Bill To:</b></p>
                    <b class="tm_primary_color"><?= $branchOrdersFor['name']; ?></b>
                  <p>
                    <b class="tm_f16 tm_primary_color">
                      <?= $branchOrdersFor['company']; ?>
                    </b> <br>
                    <?= $branchOrdersFor['address']; ?><br>
                    <?= $branchOrdersFor['email']; ?><br>
                    <?= $branchOrdersFor['phone']; ?>
                  </p>
              <?php } ?>
            </div>
                ...
                <table>
                  <thead>
                    <tr>
                      ...
                      <th class="tm_width_7 tm_semi_bold tm_primary_color">Description</th>
                      <th class="tm_width_2 tm_semi_bold tm_primary_color">HSN/SAC</th>
                      <th class="tm_width_1 tm_semi_bold tm_primary_color">Unit Cost</th>
                      <th class="tm_width_1 tm_semi_bold tm_primary_color">Quantity</th>
                      <th class="tm_width_2 tm_semi_bold tm_primary_color tm_text_right">Line Total</th>
                    </tr>
                  </thead>
                  <tbody>
                        <tr>
                            <td class="tm_width_2"><?= "B2B Services"; ?></td>
                            <td class="tm_width_7">
                                B2B Services for <?php if(count($lrs) > 1){ echo date("d-M-y",strtotime($startDate)); ?> to <?= date("d-M-y",strtotime($endDate)); }else{ echo 'LR No: '.$getreport[0]['lr']; } ?>
                            </td>
                            <td class="tm_width_2"><?= "996719"; ?></td>
                            <td class="tm_width_1">₹<?= round($unitCost, 2); ?><br></td>
                            <td class="tm_width_2 tm_text_center"><?= $actualTotalQty; ?></td>
                            <td class="tm_width_2 tm_text_center">₹<?= round($subTotalCost, 2); ?></td>
                        </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div class="tm_invoice_footer">
              ...
              <div class="tm_right_footer">
                ...
                <table style="margin-top: 8px;">
                  <tbody>
                    <tr>
                      <td class="tm_width_3 tm_border_top_0 tm_bold tm_f16 tm_white_color tm_radius_6_0_0_6" style="background-color: #266f6b;">
                        Paid amount</td>
                      <td class="tm_width_3 tm_border_top_0 tm_bold tm_f16 tm_white_color tm_text_right tm_radius_0_6_6_0" style="background-color: #266f6b;">
                        ₹<?php if($entityType == "users" && $entity['party_type'] == "All"){ echo $grandTotal; }elseif($entityType == "users" && $entity['party_type'] == "TBB"){ echo 0; }else{ echo $grandTotal; } ?>
                      </td>
                    </tr>
                  </tbody>
                </table>
                <table style="margin-top: 8px;">
                  <tbody>
                    <tr>
                      <td class="tm_width_3 tm_border_top_0 tm_bold tm_f16 tm_white_color tm_radius_6_0_0_6" style="font-size: 13px; background-color: #b3b3b3;">
                        Balance amount</td>
                      <td class="tm_width_3 tm_border_top_0 tm_bold tm_f16 tm_text_right tm_white_color tm_radius_0_6_6_0" style="background-color: #b3b3b3;">
                        ₹<?php if($entityType == "users" && $entity['party_type'] == "All"){ echo 0; }elseif($entityType == "users" && $entity['party_type'] == "TBB"){ echo $grandTotal; }else{ echo 0; }; ?>
                      </td>
                    </tr>
                  </tbody>
                </table>
          ...
          <div class="tm_note tm_text_center tm_m0_md">
            <p class="tm_m0">
              ...
              service for <?= $name." (".$uname.") to GST ID: "; if($entity['gst_type'] == "Regular" && $entityType == "users"){ echo $entity['gst_number']; }elseif($entityType == "branches"){ echo ($getreport[0]['payment_mode'] == "Prepaid" || $getreport[0]['payment_mode'] == "CoD")? $getreport[0]['seller_gst_tin'] : $getreport[0]['consignee_gst_tin']; } ?> from Kingfish <br>
              GST ID:02IRJPS0802Q1ZE, STATE CODE: <?= $getstate['state_code']; ?>, PLACE OF SUPPLY: <?= $getstate['state']; ?> <br>
              Please download the charge calculator from below given link: <a target="_blank" href="https://b2b.kingfishlogistics.in/billing-charge?getLRs=<?= $_GET['bill_report_LRs']; ?>" style="text-decoration: underline; cursor: pointer;">click here</a>.
            </p>
            <p class="tm_m0">
  ...
  <script src="https://b2b.kingfishlogistics.in/invoice/assets/js/jquery.min.js"></script>
  <script src="https://b2b.kingfishlogistics.in/invoice/assets/js/jspdf.min.js"></script>
  <script src="https://b2b.kingfishlogistics.in/invoice/assets/js/html2canvas.min.js"></script>
  <script src="https://b2b.kingfishlogistics.in/invoice/assets/js/main.js"></script>
</body>
</html>