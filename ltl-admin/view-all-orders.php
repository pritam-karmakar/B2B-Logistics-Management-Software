<?php
if(!empty($_GET) && !array_key_exists('visible', $_GET) && !array_key_exists('page', $_GET)){
    echo '<script type="text/javascript" language="javascript">window.location="view-all-orders";</script>';
}else{
    extract($_GET);
}
...
<div class="content-body">
	<div class="container-fluid">
	    <form action="view-all-orders" method="GET" class="row mb-3">
	        ...
	        <div class="col-xl-4 col-sm-6 form-group">
	            ...
	            <select class="form-control form-control-sm" id="single-select" name="orderUsersorBranches">
	               ...
	               ?>
	                    <option value="<?= $uName; ?>" <?php if(!empty($orderUsersorBranches) && $orderUsersorBranches == $uName){ echo 'selected'; } ?>><?= "Name: ".$pName.", Username: ".$uName." ( mobile no.: ".$user['mobile_no']." )"; ?></option>
	               <?php
    	                        }
    	                    }
	                    }
                                ...
                                <?php
                                  ...
                                  foreach($order_details as $orders){
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <a type="button" data-bs-toggle="modal" title="View order details" data-bs-target=".bd-example-modal-lg<?= $orders['id']; ?>" class="btn btn-info shadow btn-xs sharp me-1"><i class="bi bi-binoculars-fill"></i></a>
                                            <a href="act?deleteLr=<?= $orders['lr']; ?>&submittedBy=admin" onclick="return confirm('Are you sure to want to delete this LR?')" class="btn btn-danger btn-xs shadow sharp me-1"><i class="bi bi-trash3-fill"></i></a>
                                        </td>
                                        <td class="text-center">
                                            <?php
