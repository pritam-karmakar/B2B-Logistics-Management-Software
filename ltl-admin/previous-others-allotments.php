    ...
    <div class="container-fluid">
        <div class="card">
	        <div class="card-header">
	            <h3 class="card-title">All Previous <?= $other; ?> Allotments <?php if(!empty($username)){ ?><span style="text-transform: lowercase;"> of <?= $visible."</span> (".$username.")"; ?><?php } ?></h3>
	        </div>
	        <div class="card-body">
                	...
                	<table id="example3" class="display" style="width:100%">
                		...
                		<tbody>
                		    <?php
                		        ...
                		        $allots = $query->getData('*',$other.'_stationary_allotments','',$arr,'id','DESC','');
                		        if($allots != 0){
                        				    ...
                        				    <?php endif;
                        				          if($getallots['number_status'] == "Not Started"): ?>
                            				        <form action="act" method="POST" onclick="return confirm('Are you sure to delete this alloted numbers?')">
                                                        <input type="hidden" name="stationary_type" value="<?= $other; ?>" readonly>
                            				            <input type="text" hidden name="allotedNumbers" value="<?= $getallots['id']; ?>" />
                                    				    <button type="submit" name="delPrevOthAllot" class="btn btn-danger btn-xs sharp me-1"><i class="bi bi-trash3-fill"></i></button>
                                    			    </form>
                        				    <?php endif;
                                                ...
                                                <div class="modal-content">
                                                    <form action="act" method="POST">
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="form-group">
                                                                ...
                                                                <select <?php if($getallots['number_status'] == "Not Started"){ ?> name="stationary" <?php }else{ ?> disabled <?php } ?> class="form-control form-control-sm" required>
                                                                    ...
                                                                    <?php
                                                                        $Arr = array("stationary_name"=>ucwords($other),"delete_status"=>"show");
                                                                        $getstn = $query->getData("*","other_stationaries","",$Arr,"","","");
                                                                        foreach($getstn as $stn){
                                                                    ?>
                                                                            <option value="<?= $stn['id']; ?>" <?php if($stn['id'] == $getallots['other_stationary_id']){echo "selected";} ?>><?= "{$stn['stationary_name']} - ({$stn['stationary_prefix']})"; ?></option>
                                                                    <?php
                                                                        }
                                                                    ?>
                                                                </select>
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            <button type="submit" name="changeOtherStationary" class="btn btn-warning btn-sm">Save Changes</button>
                                                        </div>
                                                    </form>
                                                </div>
                                            </div>
